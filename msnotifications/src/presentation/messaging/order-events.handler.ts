...
export class OrderEventsHandler {
    ...
    async handleOrderStatusChanged(event: any): Promise<void> {
        try {
            ...
            const command = new CreateNotificationCommand(
                event.customerId?.toString() || `customer-${event.orderId}`,
            ...
            await this.sendNotificationUseCase.execute(command);
    ...
    async handleOrderCancelled(event: any): Promise<void> {
        try {
            ...
            const command = new CreateNotificationCommand(
                ...
                'CANCELLED',
                'orders',
                `Pedido #${event.orderId} foi cancelado. Motivo: ${event.reason}`,
            );
    ...
    async handleNotification(event: any): Promise<void> {
        try {
            ...
            const command = new CreateNotificationCommand(
                event.userId || `order-${event.orderId}`,
                event.orderId.toString(),
                event.status,
                event.serviceOrigin || 'tracking',
                event.message,
            );
